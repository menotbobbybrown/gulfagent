...
async def get_orchestrator_status(user_id: UUID = Depends(get_current_user_id)):
    # In a real app, check if user is admin. For now, assume authenticated users can see this.
    stats = orchestrator.get_stats()
    return {
        "data": {
            "available_routes": list(MODEL_ROUTES.keys()),
            "cost_today_usd": orchestrator.get_cost_today(),
            "cost_month_usd": orchestrator.get_cost_this_month(),
            **stats
        },
        "error": None
    }
